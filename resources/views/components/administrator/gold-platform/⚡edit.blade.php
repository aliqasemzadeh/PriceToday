...
new class extends Component
{
    ...
    public ?int $goldPlatformId = null;
    ...
    public function assignData(int $goldPlatformId): void
    {
        ...
        GoldPlatformFormData::loadFromModel($this, $platform);

        $this->resetValidation();

        $this->modal('administrator-gold-platform-edit')->show();
    }
    ...
    public function save(): void
    {
        ...
        $this->modal('administrator-gold-platform-edit')->close();
        $this->dispatch('panels.administrator.gold-platform.saved');

        Flux::toast(__('price-today.administrator.gold_platforms.updated_success'));
    }
};
?>

<flux:modal name="administrator-gold-platform-edit" flyout position="right" class="max-w-2xl">
    <form wire:submit="save" class="space-y-6">
        <div>
            <flux:heading size="lg">{{ __('price-today.administrator.gold_platforms.edit_title') }}</flux:heading>
            <flux:text class="mt-2">{{ __('price-today.administrator.gold_platforms.edit_description') }}</flux:text>
        </div>
